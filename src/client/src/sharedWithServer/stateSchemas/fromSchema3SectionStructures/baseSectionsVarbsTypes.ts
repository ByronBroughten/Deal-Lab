...
export type BaseSectionVarbs<SN extends SectionName> =
  Schema2SectionStructures[SN];
...
const allVarbNamesRepeats = sectionNames.reduce((allVarbNames, sectionName) => {
  const varbNames = Obj.keys(schema2SectionStructures[sectionName]);
...
export function getBaseVarb<SN extends SectionName, VN extends VarbName<SN>>(
  ...
  return schema2SectionStructures[sectionName][varbName];
}
...
export function sectionVarbNames<SN extends SectionName>(
  ...
  return Obj.keys(schema2SectionStructures[sectionName]);
}
