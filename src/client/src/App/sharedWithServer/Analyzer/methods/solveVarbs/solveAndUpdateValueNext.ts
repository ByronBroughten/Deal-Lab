...
import { isCalculationName } from "../../../SectionsMeta/baseSections/baseValues/calculations";
...
class ValueSolver extends UpdaterSections {
  solveAndUpdateValue(feVarbInfo: FeVarbInfo) {
    const newValue = this.solveValue(feVarbInfo);
    if (newValue !== undefined) this.updateValueDirectly(feVarbInfo, newValue);
  }
}

export function solveValueNext(
  analyzer: Analyzer,
