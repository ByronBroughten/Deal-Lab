...
import { BaseNameSelector } from "../baseSectionsDerived/baseNameArrs";
...
// delete the childNames from relSections;

type ChildNameArr<SN extends SimpleSectionName> = (keyof ChildSections[SN])[];
...
export const sectionToChildTypes = simpleSectionNames.reduce(
  (arrs, sectionName) => {
    ...
    const childSection = childSections[sectionName] as GeneralChildSection;
    const types = childNames.map(
      (childName) => childSection[childName].sectionName
    );
...
type RelChildren<SN extends SimpleSectionName> = ChildSections[SN];
type RelChild<
...
// This is supposed to collect all the sectionNames of the children
// and then show an array of those sectionNames

type Test = ChildTypesToNames<"property">;

export type ChildTypeName<
  SN extends SimpleSectionName,
  ST extends BaseNameSelector
> = PropKeysOfValue<ChildNamesToTypes<SN>, ChildType<SN>> & BaseName<ST>;
...
function childNamesToTypes<SN extends SimpleSectionName>(
  ...
  return childNames.reduce((toTypes, childName) => {
    (toTypes as any)[childName] = childSection[childName]["sectionName"];
    return toTypes;
