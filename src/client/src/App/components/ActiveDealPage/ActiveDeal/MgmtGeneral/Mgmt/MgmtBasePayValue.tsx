...
function getEqualsValue(
  getter: GetterSection<"mgmtBasePayValue">
): string | undefined {
  const valueSourceName = getter.valueNext("valueSourceName");
  if (valueSourceName === "percentOfRentEditor") {
    return getter.activeSwitchTarget("valueDollars", "ongoing").displayVarb();
  } else if (valueSourceName === "dollarsEditor") {
    return getter.varbNext("valuePercent").displayVarb();
  } else {
    return undefined;
  }
}

function getEditorVarbInfo(
  getter: GetterSection<"mgmtBasePayValue">
): FeVarbInfo | undefined {
  const valueSourceName = getter.valueNext("valueSourceName");
  if (valueSourceName === "percentOfRentEditor") {
    return getter.varbNext("valueDollarsOngoingEditor").feVarbInfo;
  } else if (valueSourceName === "dollarsEditor") {
    return getter.varbNext("valuePercentEditor").feVarbInfo;
  } else {
    return undefined;
  }
}
